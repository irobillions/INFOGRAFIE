...
BasicShapeArrays::BasicShapeArrays(const GLfloat* data, GLsizeiptr byteSize)
{
    ...
    // empecher le programme de modifier les tampons precedement lier
...
void BasicShapeArrays::enableAttribute(GLuint index, GLint size, GLsizei stride, GLsizeiptr offset)
{
    ...
    glBindVertexArray(m_vao);
    glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
    glVertexAttribPointer(index, size, GL_FLOAT, GL_FALSE, stride, (void*)offset); // setup l' attribut de position a parametrer pour le VBO (matching pour savoir ou lire)
